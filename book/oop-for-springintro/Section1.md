...
#### 신기술은 이전 기술의 어깨를 딛고 개발자를 위해 발전했다.

- 기계어 : 개발자가 사랑하기엔 너무 초창기 기술
- 어셈블리어 : 인간의 언어로 프로그램 작성 가능
- C : 하나의 소스로 기종간 이식성을 확보
- C++ : 객채 지향 개념을 도입 ➡️ 인간적인 사고의 프로그래밍
- 자바 : 한번의 컴파일로 기종간 이식성을 확보
  - 포인터 개념없이 프로그래밍 가능
- **스프링 프레임 워크**
  - 아무리 거대한 프로그램이라도 쉽게 구현할 수 있음을 보여준 프레임워크
  - IoC/DI, AOP, PSA ➡️ 객체 지향의 Best Practice

#### 신기술이 역사 속에서 환영만 받은 것은 아니다.

- C : 기계어/어셈블리어보다 느렸다. 당시 CPU/메모리 자원을 효율적으로 사용하지 못함.
- C++ : 기존 절차적/구조적 프로그래밍과 전혀 다른 객체지향 패러다임으로의 전환에 대한 저항이 있었음.
- Java : JVM은 가상의 컴퓨터를 물리적 컴퓨터의 메모리 안에 하나 더 구축하는 것.
  - 물리적으로 더 많은 CPU자원과 메모리를 소비하여 속도가 굉장히 느렸다.
  - 현재는 하드웨어의 발전, 구성 요소의 가격하락, 최적화된 알고리즘으로 빨라짐

<hr/>

### 짧은 글, 긴 생각
#### 당신은 CBD, SOA가 어려운가?
- CBD(Componet Based Development) : 컴포넌트 기반 개발.
  - 애플리케이션을 의미 있는 단위로 구분하고 그 단위를 부품으로 개발해 레고 블록 쌓아 올리듯 부품을 결합 해 SW를 완성하자는 방법론.
- SOA(Service Oriented Architecture) : 서비스 지향 구조
  - 개발이 아닌, 실제 업무를 기주능로 개발하자는 사상.
- 개념은 있는데 이를 대표하는 제품이 없다. 설명해줄 메타포도 없으니 어렵다.

#### 스프링 프레임워크는 사상이면서 또 단일 제품이다.
- 스프링 프레임워크는 **OOP프레임워크**다.
  - OOP프레임워크 with IoC, DI, AOP, PSA, WEB, ORM, OXM, Security, Test, IBatis, Hibernate, JPA, ... on POJO, SOLID, OOP Design Patterns
- 스프링을 지배하는 근원적 요소 = 스프링 삼각형
  - **IoC/DI, AOP, PSA** on POJO(Plain Old Java Object)
