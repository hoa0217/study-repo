...
![image](https://github.com/f-lab-edu/modoospace/assets/48192141/92e26d5b-8f8c-424e-98c3-b26ac398bbeb)
...
> 마찬가지로 모든 화살표는 main에서 애플리케이션쪽으로 향한다. 애플리케이션은 객체가 생성되는 구체적인 방법은 모르며 이는 오로지 LineItemFactory구현체만 알뿐이다.
> 
...
```
위 예시에서 프록시로 감쌀 인터페이스 Bank와 비지니스 논리를 구현하는 POJO로 BankImpl을 정의했다.

InvocationHandler를 프록시 API에 넘겨서, 프록시에 호출되는 Bank메서드를 BankImpl에 매핑한다.

단점
- 코드의 양과 크기가 너무 크며 복잡하다.
  - 깨끗한 코드를 작성하기 어렵다. 
- 시스템 단위로 실행 '지점'을 명시하는 메커니즘도 제공하지 않는다.

### 순수 자바 AOP 프레임워크

다행히 대부분의 프록시 코드는 도구로 자동화 할 수 있다. 스프링 AOP, JBoss AOP 등 여러 자바 프레임워크는 내부적을 프록시를 사용한다.
- 스프링은 비즈니스 논리를 다른 엔터프라이즈 프레임워크에 의존하지않고 POJO로 구현하여 순수한 도메인에 초점을 맞춘다. 따라서 테스트도 쉽고 유지보수도 편하다.

스프링 AOP는 설정파일로 어플리케이션에 필요한 구조와 횡단관심사(영속성, 트랜잭션, 보안 등)를 선언해놓으면, 사용자 모르게 프록시나 바이트코드 라이브러리를 사용해 이를 구현한다.
- DI 컨테이너가 객체의 생성과 주입을 알아서 처리한다.
...
만약 해당 Bean을 DI컨테이너에게 요청하고 싶다면 아래와 같은 코드가 필요하다.
...
- xml이 읽기 어려울지라도, 자바 동적 프록시보다 설정이 단순하다.
