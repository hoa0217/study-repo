...
<img src="img/DIP1.png" width="300"/>
...
> 이 경험적 접근 방법은 다소 고지식하다.
> 1. 자주 바뀌지 않는 비휘발적인 클래스(ex. String)엔 적용할 이유가 없다. 직접 의존하는 것은 해가 되지 않는다.
> 2. 휘발적인 구체 클래스를 변경할 경우, 이 변경은 추상 인터페이스로 전파되어 분리 상태를 망가뜨린다.
> 
> 그러나 클라이언트 클래스가 자신이 필요로하는 서비스 인터페이스를 선언한다는 장기적 관점을 택한다면, 이 인터페이스가 변경되는 경우는 오직 클라이언트가 변경을 필요로 할 때가 된다. 추상 인터페이스를 구현하는 클래스의 변경은 클라이언트에 영향을 주지 않는다.

### 결론
#### 절차 지향 프로그래밍 
- 정책이 구체적인 사항에 의존하는 의존성 구조를 만든다.
- 이런 경우 정책은 구체적인 사항의 변경에 따라 같이 변하기 때문에 불행한 일이다.

#### 객체 지향 프로그래밍
- 의존성 구조를 역전시켜 정책과 구체적인 사항 모두 추상화에 의존하게 한다.
- 대개 클라이언트가 서비스 인터페이스를 소유하게 만든다.

즉, 프로그램의 의존성이 역전되어 있다면 객체 지향 설계이며, 역전되어 있지 않다면 절차적 설계다.

> - 재사용 가능한 프레임워크를 만들기 위해 DIP의 적절한 응용은 필수다.   
> - 추상화와 구체적 사항이 서로 분리되어 있기 때문에, 이 코드는 유지보수하기 훨씬 쉽다.
